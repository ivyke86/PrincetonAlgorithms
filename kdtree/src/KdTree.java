...
public class KdTree {
	...
	private static class Node {
		...
		private Node rt ;
		private boolean isH ;
		public Node( Point2D point, RectHV r, boolean H ) {
			p = point ;
			rect = r ;
			isH = H ;
		}
	...
	public void insert( Point2D p ) {
		...
		if( cmp == 0 && cmp2 == 0 ) return ;
		
		if( cmp < 0 ) {
			root.lb = put( root.lb, p, root, lbrt.LEFT );
		} else {
			root.rt = put( root.rt, p, root, lbrt.RIGHT );
		}
		
		
	}
	
	private Node put( Node x, Point2D p, Node bN, lbrt drct ) {
		if( x == null ) { 
			...
			else if( drct == lbrt.RIGHT ) {
				nRect = new RectHV( bPoint.x(), ymin, xmax, ymax ) ;
			}
			else if( drct == lbrt.BOTTOM ) {
			...
			size++ ;
			return new Node( p, nRect, drct == lbrt.RIGHT || drct == lbrt.LEFT ) ;
		}
			
		if( x.p == p ) return x ;
		int cmp, cmp2 ;
		if( drct == lbrt.BOTTOM || drct == lbrt.TOP ) {
		...
		if( cmp == 0 && cmp2 == 0 ) return x;
		
		if( cmp < 0 ) {
			x.lb = put( x.lb, p, x, drct == lbrt.LEFT || drct == lbrt.RIGHT ? lbrt.BOTTOM : lbrt.LEFT );
		} else {
			x.rt = put( x.rt, p, x, drct == lbrt.LEFT || drct == lbrt.RIGHT ? lbrt.TOP : lbrt.RIGHT );
		}
		return x;
	}
	...
	private void range( Node x, RectHV rect, Stack<Point2D> st ) {
		...
		if( rect.contains(x.p) ) st.push(x.p);
